...
function getClient(explicitKey = null) {
  const apiKey = explicitKey || process.env.AI_OPENAI_API_KEY || process.env.AI_API_KEY;
  if (!apiKey) {
    throw new Error('AI_OPENAI_API_KEY or AI_API_KEY environment variable not set.');
  }
...
async function chatCompletion(messages, options = {}) {
  ...
  const model = options.model || process.env.AI_OPENAI_MODEL || process.env.AI_MODEL;
