...
class VectorStoreManager {
  ...
  async syncFromDB() {
    ...
    const changedArticles = await HochschuhlABC.findMany({ where: { updated_at: { gt: this.lastSync }, pdf_path: { not: null } } });
    for (const a of changedArticles) {
      if (a.active) {
        ...
        if (a.pdf_path) {
          ...
          const loader = new PDFLoader(a.pdf_path);
          const pdfDocs = await loader.load();
          for (const doc of pdfDocs) {
            let pageContent = `${a.article}\n${a.description || ''}\n${doc.pageContent}`;
            pageContent = sanitizeHtml(pageContent);
    ...
    logger.info(`Synced ${stats.chunks} chunks from ${stats.headlines} headlines, ${stats.pdfs} PDFs, and ${stats.images} images`);
    // Update last sync
