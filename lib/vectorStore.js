...
class VectorStoreManager {
  constructor() {
    const modelName = process.env.EMBEDDING_MODEL ? `Xenova/${process.env.EMBEDDING_MODEL}` : 'Xenova/all-MiniLM-L6-v2';
  ...
  async syncFromDB() {
    ...
    const changedHeadlines = await HochschuhlABC.findAll({ where: { updated_at: { [Op.gt]: this.lastSync } }, attributes: ['id', 'headline', 'text', 'active'] });
    ...
    // Fetch changed articles with PDFs
    const changedArticles = await HochschuhlABC.findAll({ where: { updated_at: { [Op.gt]: this.lastSync }, pdfPath: { [Op.ne]: null } } });
    for (const a of changedArticles) {
      if (a.active) {
        // Delete old
        await this.store.delete({ filter: { $and: [{ source: 'pdf' }, { articleId: a.id }] } });
        // Add new
        if (a.pdfPath) {
    ...
    // TODO: Add incremental sync for images once DB schema is confirmed
