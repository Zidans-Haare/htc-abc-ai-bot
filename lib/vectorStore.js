...
class VectorStoreManager {
  ...
  async syncFromDB() {
    ...
    const changedHeadlines = await HochschuhlABC.findMany({ where: { updated_at: { gt: this.lastSync } }, select: { id: true, headline: true, text: true, active: true } });
    ...
    const changedArticles = await HochschuhlABC.findMany({ where: { updated_at: { gt: this.lastSync }, pdfPath: { not: null } } });
