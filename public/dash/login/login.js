document.addEventListener('DOMContentLoaded', async () => {
  console.log('Dashboard login page loaded, initializing...');
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.role === 'admin') {
          ...
          alert('Zugriff verweigert: Sie haben keine Berechtigung, auf das Dashboard zuzugreifen.');
        }
      } else {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      if (data.role === 'admin') {
        console.log('Session valid and user is admin, redirecting to dashboard...');
        window.location.href = '/dash';
      } else {
